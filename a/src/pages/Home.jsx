...
import {
  current as sampleCurrent,
  todayTrend as sampleTrend,
  toneForValue,
  labelForValue,
  colorForValue,
  filterStatus,
...
export default function Home() {
  ...
  const { latest: firebaseLatest, history: firebaseHistory, loading } = useFirebaseData()

  // ใช้ข้อมูลจาก Firebase ถ้ามี ไม่งั้น fallback ไป sample data
  const current = firebaseLatest
    ? { before: firebaseLatest.ntu ?? sampleCurrent.before, after: sampleCurrent.after }
    : sampleCurrent

  const todayTrend = firebaseHistory.length > 0
    ? firebaseHistory.map((d) => ({
        time: d.timestamp ? new Date(d.timestamp * 1000).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit', hour12: false }) : '-',
        ntu: d.ntu ?? 0
      }))
    : sampleTrend
