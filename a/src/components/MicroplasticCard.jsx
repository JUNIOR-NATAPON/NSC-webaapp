...
function levelForClass(cls) {
  if (!cls) return null
  if (cls === 'low') return { tone: 'clean', label: 'Low' }
  if (cls === 'medium') return { tone: 'warn', label: 'Medium' }
  return { tone: 'danger', label: 'High' }
}
...
export default function MicroplasticCard({
  ...
  const { latest } = useFirebaseData()
  const isDesktop = useIsDesktop()
  const gaugeWidth = isDesktop ? 96 : 76
  const gaugeHeight = isDesktop ? 137 : 108

  // ถ้ามีข้อมูลจาก Firebase ให้ใช้แทน mock
  const hasRealData = latest && latest.particles_per_L !== undefined
  const particlesPerL = hasRealData ? latest.particles_per_L : null
  const aiClass = hasRealData ? latest.class : null
  const aiLevel = levelForClass(aiClass)

  const level = aiLevel || levelFor(accumulatedMg)
  ...
  return (
    <Card className="md:col-span-2">
      ...
      {/* แสดงข้อมูลจาก AI ถ้ามี */}
      {hasRealData && (
        <div className="mb-4 p-3 rounded-xl bg-black/5 dark:bg-white/5 flex flex-wrap gap-4 text-sm">
          <div>
            <span className="text-muted dark:text-muted-dark">AI Detection · </span>
            <span className="font-bold">{particlesPerL.toFixed(1)}</span>
            <span className="text-muted dark:text-muted-dark ml-1">particles/L</span>
          </div>
          <div>
            <span className="text-muted dark:text-muted-dark">Class · </span>
            <span className="font-bold capitalize">{aiClass}</span>
          </div>
          {latest.timestamp && (
            <div className="text-muted dark:text-muted-dark">
              Last updated · {new Date(latest.timestamp * 1000).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })}
            </div>
          )}
        </div>
      )}
